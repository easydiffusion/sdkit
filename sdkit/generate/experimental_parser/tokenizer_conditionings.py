...
def get_vocabulary_size(encoder_model):
    ...
    log.warn(
        f"{encoder_model.__class__.__name__} has no vocab_size/encoder and/or is not implemented. Returning {DEFAULT_VOCABULARY_SIZE} as default."
    )
...
def get_cond_and_uncond(prompt, unconditional_prompt, batch_size, model, **kwargs):
    ...
    if "unconditional_transforms" not in kwargs:
        ...
        if unconditional_prompt.startswith("**"):  # Random float to tensors.
            ...
            log.info(
                f"Unconditional conditioning set to random, returning {max_tokens_len}x{max_dimensions_len} random tensors."
            )
            unconditional_conditioning = get_random_conditioning(
                max_tokens_len, max_dimensions_len, model.device, rndSeed
            )
            ...
            log.info(
                f"Returning new prompt text of {max_len} tokens using random sequence {rndSeed} with a vocabulary of {vocab_size} tokens."
            )
                ...
                if "slerp" not in transform:
                    ...
                    for i in range(len(tokens_ids)):
                        if np.array_equal(tokens_ids[i : i + len(ids)], ids):
                            ...
                            log.info(
                                f'{len(transform["transforms"])} transforms[{i}:{i + len(ids)}] applied to "{text}" {tokens}'
                            )
                            break
                    else:
...
def print_text_conditioning(model, text, tokens, ids, conditioning):
    if hasattr(model.cond_stage_model, "tokenizer") and model.cond_stage_model.tokenizer:
        ...
        tokens = [
            ...
            != (
                tokenizer.pad_token
                if idx < len(tokens) - 1 and hasattr(tokenizer, "pad_token")
...
def get_random_conditioning(nbr_tokens, nbr_dimensions, device, seed=None, alpha=0.25):
    ...
    rnd_cond = torch.rand(
        tuple([int(nbr_tokens - 2), int(nbr_dimensions)]), generator=generator.manual_seed(seed), device=device
    )
