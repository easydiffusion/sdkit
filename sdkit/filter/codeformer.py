...
def apply(context: Context, input_img, background_enhance=False, face_upsample=True, rescaling_factor=4, codeformer_fidelity=0, **kwargs):
    ...
    input_img = np.array(input_img)import sys
import os
import cv2
import torch
import torch.nn.functional as F
...
def CodeFormer_Initialize(model_dir):
    pretrain_model_url = {
        'codeformer': 'https://github.com/sczhou/CodeFormer/releases/download/v0.1.0/codeformer.pth',
        'detection': 'https://github.com/sczhou/CodeFormer/releases/download/v0.1.0/detection_Resnet50_Final.pth',
        'parsing': 'https://github.com/sczhou/CodeFormer/releases/download/v0.1.0/parsing_parsenet.pth',
...
def apply(context: Context, input_img, background_enhance=False, face_upsample=True, rescaling_factor=4, codeformer_fidelity=0, **kwargs):
    ...
    imread, set_realesrgan, upsampler, device, codeformer_net, upsampler = CodeFormer_Initialize(model_dir)
