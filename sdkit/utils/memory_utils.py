"""
    Utilities for memory management and debugging memory leaks.
"""
import torch
import psutil
import base64
...
def gc(context: Context):
    """
        Collects garbage and frees up GPU memory.
    """
    collect()
...
def get_device_usage(device, log_info=False):
    """
        Returns the CPU and GPU memory usage of the current process.
    """
    cpu_used = psutil.cpu_percent()
    ram_used, ram_total = psutil.virtual_memory().used, \
    ...
    if log_info:
        ...
        # TODO: Lookup percent escape.
        log.info(
            'CPU utilization percent: %s, System RAM used: %s of %s GiB',
            f"{cpu_used:.1f}", f"{ram_used:.1f}", f"{ram_total:.1f}"
        )
        if device != 'cpu':
            log.info(
                'GPU RAM used (%s): %s of %s GiB',
                device, f"{vram_used:.1f}", f"{vram_total:.1f}"
            )
...
def record_tensor_name(t, name='t', log_info=False):
    '''
    Records a name for the given tensor object. 
        Helpful while investigating the source of memory leaks.

    For e.g. you can record variables from across the codebase,
         and see which one is leaking by calling
        `print_largest_tensors_in_memory()` or `take_memory_snapshot()`
            after calling `gc()` (for garbage-collection).
    ...
    recorded_tensor_names[obj_id] = [] if obj_id not in recorded_tensor_names \
        else recorded_tensor_names[obj_id]
...
def print_tensor_info(t, name='t'):
    '''
        Prints a summary of the tensor,
          for e.g. its size, shape, data type, device etc.
    '''
    ...
    log.info(
        ' %s id: %s, size: %s MiB, shape: %s, '
        'requires_grad: %s, type: %s, device: %s',
        name, obj_id, obj_size, t.shape, t.requires_grad, t.dtype, t.device
    )
...
def get_tensors_in_memory(device):
    ...
    **  Warning: Do not keep a reference to the returned list longer than
    **  necessary, since that will prevent garbage-collection of all the
    **  tensors in memory.
    '''
    from sdkit.utils import log
    ...
    for obj in objs_in_mem:
        try:
            ...
            log.warning('Error while getting tensors in memory: %s', ex)
...
def print_largest_tensors_in_memory(device, num=10):
    ...
    Prints the recorded names for each tensor,
        if recorded using `record_tensor_name()`.

    See also: `take_memory_snapshot()` which is probably more useful for
    ...
    print('---')
    print(f'Total memory occupied on {device} by'
          f' {n} tensors: {total_mem:.1f} MiB')


def take_memory_snapshot(device, print_snapshot=True):
    '''
    Records and prints a list of new tensors (in the device) since the
        last snapshot (created by calling `take_memory_snapshot()`).
    ...
    '''
    # TODO: Use contextvar instead, avoid global state.
    ...
    new_tensor_entries = [
        entry for entry in entries if entry[0] in new_tensor_ids
    ]
    ...
    print('---')
    print(f'Total memory occupied on {device} by {len(entries)} '
          f' tensors: {total_mem:.1f} MiB')
    print(f'{num_new_tensors} new tensors added {new_tensors_total_mem:.1f}'
          f' MiB this frame')
...
def _get_tensor_entries(device, sorted_by_size=True):
    ...
    for t in tensors:
        ...
        obj_id = get_object_id(t)
        entry = [
            obj_id, size, t.shape, len(get_referrers(t)),
            t.requires_grad, t.dtype
        ]
...
def _fmt_tensors_summary(entries):
    ...
    for i, o in enumerate(entries):
        ...
        known_names = f' ({recorded_tensor_names[obj_id]})' \
            if obj_id in recorded_tensor_names else ''
        summary.append(f'{i+1}. Id: {obj_id}{known_names}, Size: {size:.1f} '
                       f'MiB, Shape: {shape}, Referrers: {n_referrers}, '
                       f'requires_grad: {requires_grad}, dtype: {dtype}')
