"""
Merges two models using weighted sum.
...
def merge_models(
    ...
    * ratio - the ratio of the second model.
        1 means only the second model will be used.
        0 means only the first model will be used.
    '''

    log.info('[cyan]Merge models:[/cyan] Merging %s and %s, ratio %s', 
             model0_path, model1_path, ratio)
    merged = merge_two_models(model0_path, model1_path, ratio, use_fp16)

    log.info('[cyan]Merge models:[/cyan] ... saving as %s', out_path)

    if out_path.lower().endswith(".safetensors"):
        # elldrethSOg4060Mix_v10.ckpt contains a state_dict
        #  key among all the tensors, but safetensors
...
def merge_two_models(model0, model1, alpha, use_fp16=True):
    ...
    * model0, model1 - the first and second model files to be merged
    * alpha - a float between [0, 1].
        0 means only model0 will be used,
        1 means only model1.
    
    If model0 is a tensor, then model0 will be over-written with the
        merged data, and the same model0 reference will be returned.
    '''

    model0_file = load_tensor_file(model0) \
        if isinstance(model0, str) else model0
    model1_file = load_tensor_file(model1) \
        if isinstance(model1, str) else model1

    model0 = model0_file['state_dict'] \
        if 'state_dict' in model0_file else model0_file
    model1 = model1_file['state_dict'] \
        if 'state_dict' in model1_file else model1_file
