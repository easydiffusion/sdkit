"""
    Downloads the requested models (and config files)
     based on the SDKit models database.
"""
import os
    ...
    * subdir_for_model_type: bool - default True. Saves the downloaded model in a subdirectory (named with the model_type).
    ...
    for model_type, model_ids in models.items():
        ...
        for model_id in model_ids:
            download_model(model_type, model_id, download_base_dir,
                           subdir_for_model_type)


def download_model(model_type: str, model_id: str,
                   download_base_dir: str=None, subdir_for_model_type=True):
    '''
    Downloads the requested model (and config file)
     based on the SDKit models database.
    Resumes incomplete downloads, and shows a progress bar.
    ...
    * download_base_dir: str
        Base directory inside which the models are downloaded.
              Note: The actual download directory path will include the
               `model_type` if `subdir_for_model_type` is `True`.
              If `download_base_dir` is `None`, it'll use the
               `~/.cache/sdkit` folder, where `~` is the home or user-folder.
    * subdir_for_model_type: bool - default True.
        Saves the downloaded model in a subdirectory.
            (named with the model_type)
              For e.g. if `download_base_dir` is `D:\\models`,
               then a `stable-diffusion` type model is downloaded to
              `D:\\models\\stable-diffusion`, a `hypernetwork`
                type model is downloaded to `D:\\models\\hypernetwork`
                 and so on.
    '''
    ...
    try:
        ...
        if model_url is None:
            log.warning(
                'No download url found for model %s %s',
                model_type, model_id
            )
            return
...
def resolve_downloaded_model_path(
    ...
    * download_base_dir: str 
        - base directory inside which the models are downloaded.
              Note: The actual download directory path will include
               the `model_type` if `subdir_for_model_type` is `True`.
              If `download_base_dir` is `None`, it'll use the `~/.cache/sdkit`
               folder, where `~` is the home or user-folder.
    * subdir_for_model_type: bool 
        - default True. Saves the downloaded model in a subdirectory
          (named with the model_type).
              For e.g. if `download_base_dir` is `D:\\models`, 
               then a `stable-diffusion` type model is downloaded to
               `D:\\models\\stable-diffusion`, a `hypernetwork` type
                model is downloaded to `D:\\models\\hypernetwork` and so on.
    '''
    download_base_dir = get_actual_base_dir(
        model_type, download_base_dir, subdir_for_model_type
    )
...
def get_actual_base_dir(model_type, download_base_dir, subdir_for_model_type):
    """
        Returns the actual base directory path,
         based on the `download_base_dir` and `subdir_for_model_type` args.
    """
    download_base_dir = os.path.join('~', '.cache', 'sdkit') \
        if download_base_dir is None else download_base_dir
    download_base_dir = os.path.join(download_base_dir, model_type) \
        if subdir_for_model_type else download_base_dir
    return os.path.abspath(download_base_dir)


def get_url_and_filename(model_type, model_id, url_key='url'):
    """
        Returns the url and filename for the given model_type and model_id.
    """
