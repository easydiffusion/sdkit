...
from sdkit.utils import load_tensor_file, save_tensor_file, hash_file_quick,\
     download_file, log
...
def load_model(
    ...
    **kwargs
    ...
    """ Load the model. """
    from sdkit.models.model_loader.stable_diffusion import optimizations
    from sdkit.models import scan_model as scan_model_fn

    model_path = context.model_paths.get('stable-diffusion')
    config_file_path = get_model_config_file(
        context, check_for_config_with_same_name
    )
    ...
    if scan_model:
        ...
        if scan_result.issues_count > 0 or scan_result.infected_files > 0:
            raise RuntimeError(
                f'Model scan failed! Potentially infected'
                f' model: {model_path}'
            )
    ...
    if config_file_path is None and \
            'cond_stage_model.model.ln_final.bias' not in sd.keys():
        ...
        config_file_path = resolve_model_config_file_path(
            sd_v1_4_info, model_path
        )

    # load the config
    if config_file_path is None:
        raise RuntimeError(
            'Unknown model! No config file path specified in'
            ' context.model_configs for the "stable-diffusion" model!'
        )

    log.info('using config: %s', config_file_path)
    ...
    attn_precision = extra_config.get(
        'attn_precision',
        'fp16' if context.half_precision else 'fp32'
    )

    log.info('using attn_precision: %s', attn_precision)
    ...
    # optimize CrossAttention.forward() for faster performance,
    # and lower VRAM usage
    ldm.modules.attention.CrossAttention.forward = \
        optimizations.make_attn_forward(context, attn_precision=attn_precision)
    ldm.modules.diffusionmodules.model.nonlinearity = silu

    # save the model vae into a temp folder 
    # (used for restoring the default VAE, if a custom VAE is unloaded)
    save_tensor_file(
...
def unload_model(context: Context, **__) -> None:
    """ Unload the model. """
    # don't keep a dangling reference, prevents gc
    context.module_in_gpu = None


def get_model_config_file(context: Context, check_for_config_with_same_name):
    """
        Get the model config file path, based on the context.
    """
    from sdkit.models import get_model_info_from_db
...
def resolve_model_config_file_path(model_info, model_path):
    """
        Resolve the model config file path, based on the model info.
    """
